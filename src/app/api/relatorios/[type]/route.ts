...
export async function GET(request: NextRequest, { params }: { params: Promise<{ type: string }> }) {
  ...
  const data = await getReportsData(store.id, range.from, range.to);
  let rows: unknown[][];
  if (type === "sales") rows = [["Produto", "Peças vendidas", "Receita"], ...data.sales.products.map((row) => [row.name, row.pieces, row.revenue])];
  else if (type === "inventory") rows = [["Produto", "Categoria", "Saldo atual", "Estoque mínimo"], ...data.inventory.products.map((row) => [row.name, row.category, row.units, row.minimum])];
  else if (type === "cash") rows = [["Forma de pagamento", "Total recebido"], ...data.cash.byPayment.map((row) => [row.label, row.value])];
  else rows = [["Cliente", "Telefone", "Saldo em aberto"], ...data.receivables.customers.map((row) => [row.name, row.phone, row.balance])];
