...
export default async function EditProductPage({ params }: PageProps<"/produtos/[id]/editar">) {
  ...
  return <><PageHeader title="Editar produto" description="Altere os dados comerciais e mantenha as variações organizadas." /><ProductForm mode="edit" options={options} initial={{ id: product.id, name: product.name, internalCode: product.internalCode, categoryName, productTypeName, supplierName, brand: product.brand ?? "", description: product.description ?? "", notes: product.notes ?? "", photoUrl: product.photoUrl ?? "", costPrice: product.costPrice, salePrice: product.salePrice, minimumStock: product.minimumStock, variants: variants.filter((item) => item.active).map((item) => ({ id: item.id, color: item.color, size: item.size, internalCode: item.internalCode ?? "", initialQuantity: item.quantityOnHand, quantity: item.quantityOnHand })) }} /></>;
}
