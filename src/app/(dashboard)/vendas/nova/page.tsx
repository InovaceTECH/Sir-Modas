...
export default async function NewSalePage() {
  ...
  const [catalog, customers] = await Promise.all([getSaleCatalog(store.id), getCustomerOptions(store.id)]);
  return <><PageHeader title="Nova venda" description="Selecione os produtos, vincule a cliente quando necessário e finalize a venda." /><SaleForm catalog={catalog} customers={customers} /></>;
}
