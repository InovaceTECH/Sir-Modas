...
import { cashSessions, products, productVariants, saleItems, sales } from "@/db/schema";
...
export async function getDashboardData(storeId: string, period: "today" | "week" | "month" = "today") {
  ...
  const [saleSummary, stockSummary, openCash, recentSales, receivables] = await Promise.all([
    getDb().select({
    ...
    getDb().select({ id: cashSessions.id, openedAt: cashSessions.openedAt }).from(cashSessions).where(and(eq(cashSessions.storeId, storeId), eq(cashSessions.status, "open"))).limit(1),
  ...
  return { sales: saleSummary[0] ?? { revenue: "0", saleCount: 0 }, stock: stockSummary[0] ?? { lowCount: 0, outCount: 0 }, receivables, openCash: openCash[0] ?? null, recentSales, pieceCount: new Map(pieces.map((item) => [item.saleId, item.quantity])) };
}
