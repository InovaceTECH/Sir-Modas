...
type CatalogItem = { variantId: string; productName: string; productCode: string; color: string; size: string; stock: number; price: string; category: string | null };
...
export function SaleForm({ catalog, customers, cashOpen }: Readonly<{ catalog: CatalogItem[]; customers: CustomerOption[]; cashOpen: boolean }>) {
  ...
  const visible = useMemo(() => {
    ...
    return catalog.filter((item) => `${item.productName} ${item.productCode} ${item.color} ${item.size} ${item.category ?? ""}`.toLocaleLowerCase("pt-BR").includes(term)).slice(0, 20);
  }, [catalog, search]);
