...
export function SaleForm({ catalog, customers }: Readonly<{ catalog: CatalogItem[]; customers: CustomerOption[] }>) {
  ...
  return <form action={action} className="grid gap-6 xl:grid-cols-[1.25fr_0.85fr]">
    ...
    <div className="space-y-6">
      ...
      <button disabled={pending || !cart.length || total <= 0 || Math.abs(paid - total) >= 0.005 || (hasOnAccount && (!customerId || !dueDate))} className="ui-button-primary w-full disabled:cursor-not-allowed disabled:opacity-45">{pending ? <LoaderCircle className="animate-spin" size={18} /> : <CreditCard size={18} />}{pending ? "Finalizando..." : `Finalizar venda · ${currency.format(total)}`}</button>
