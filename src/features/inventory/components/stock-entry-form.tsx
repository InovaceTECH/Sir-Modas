...
export function StockEntryForm({ options, suppliers }: Readonly<{ options: StockOption[]; suppliers: string[] }>) {
  ...
  const [items, setItems] = useState<Item[]>([{ key: "initial", variantId: "", quantity: 1, unitCost: 0, salePrice: 0 }]);

  function update(key: string, field: keyof Omit<Item, "key">, value: string) {
    setItems((current) => current.map((item) => {
      if (item.key !== key) return item;
      if (field === "variantId") {
        const selected = options.find((option) => option.variantId === value);
        return { ...item, variantId: value, salePrice: Number(selected?.salePrice ?? item.salePrice) };
      }
  ...
  return <form action={action} className="space-y-6">
    <input type="hidden" name="items" value={JSON.stringify(items.map((item) => ({ variantId: item.variantId, quantity: item.quantity, unitCost: item.unitCost, salePrice: item.salePrice })))} />
