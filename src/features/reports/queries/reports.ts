...
export async function getReportsData(storeId: string, from: string, to: string) {
  ...
  const [saleSummaryRows, pieceSummaryRows, productsSold, salesBySource, salesByPayment, inventorySummaryRows, inventoryProducts, inventoryMovements, cashSummaryRows, cashByPayment, closedCashRows, receivableSummaryRows, receivedSummaryRows, customerBalances, recentReceivablePayments] = await Promise.all([
    ...
    getDb().select({ received: sql<string>`coalesce(sum(case when ${cashMovements.type} in ('cancellation','withdrawal','expense') then -${cashMovements.amount} else ${cashMovements.amount} end) filter (where ${cashMovements.type} in ('sale','receivable_payment','cash_injection','adjustment','cancellation','withdrawal','expense')), 0)`, withdrawals: sql<string>`coalesce(sum(${cashMovements.amount}) filter (where ${cashMovements.type} = 'withdrawal'), 0)`, expenses: sql<string>`coalesce(sum(${cashMovements.amount}) filter (where ${cashMovements.type} = 'expense'), 0)` }).from(cashMovements).innerJoin(cashSessions, eq(cashSessions.id, cashMovements.cashSessionId)).where(cashPeriod),
    getDb().select({ label: cashMovements.paymentMethod, value: sql<string>`sum(case when ${cashMovements.type} in ('cancellation','withdrawal','expense') then -${cashMovements.amount} else ${cashMovements.amount} end)` }).from(cashMovements).innerJoin(cashSessions, eq(cashSessions.id, cashMovements.cashSessionId)).where(and(cashPeriod, sql`${cashMovements.paymentMethod} is not null`, sql`${cashMovements.type} in ('sale','receivable_payment','adjustment','cancellation','withdrawal','expense')`)).groupBy(cashMovements.paymentMethod).orderBy(desc(sql`sum(case when ${cashMovements.type} in ('cancellation','withdrawal','expense') then -${cashMovements.amount} else ${cashMovements.amount} end)`)),
...
export async function getReportExportData(storeId: string, from: string, to: string) {
  ...
  const [salesProducts, inventoryProducts, cashPayments, customerBalances] = await Promise.all([
    ...
    getDb().select({ label: cashMovements.paymentMethod, value: sql<string>`sum(case when ${cashMovements.type} in ('cancellation','withdrawal','expense') then -${cashMovements.amount} else ${cashMovements.amount} end)` })
      .from(cashMovements).innerJoin(cashSessions, eq(cashSessions.id, cashMovements.cashSessionId))
      .where(and(cashPeriod, sql`${cashMovements.paymentMethod} is not null`, sql`${cashMovements.type} in ('sale','receivable_payment','adjustment','cancellation','withdrawal','expense')`)).groupBy(cashMovements.paymentMethod).orderBy(desc(sql`sum(case when ${cashMovements.type} in ('cancellation','withdrawal','expense') then -${cashMovements.amount} else ${cashMovements.amount} end)`)),
