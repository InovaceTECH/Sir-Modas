...
export function ExchangeForm({ sale, returnableItems, catalog, outsideDeadline }: Readonly<{ sale: { id: string; number: string }; returnableItems: ReturnableItem[]; catalog: CatalogItem[]; outsideDeadline: boolean }>) {
  ...
  return <form action={action} className="space-y-6">
    ...
    <section className="grid gap-6 xl:grid-cols-2">
      ...
      <article className="ui-card p-5 sm:p-6"><p className="text-xs font-semibold uppercase tracking-wide text-muted">2. Novo produto</p><h2 className="mt-2 text-lg font-semibold">Item entregue à cliente</h2><div className="mt-5 space-y-4"><label><span className="text-sm font-semibold">Produto e variação</span><select name="deliveredVariantId" value={deliveredVariantId} onChange={(event) => setDeliveredVariantId(event.target.value)} className="ui-input mt-2">{catalog.filter((item) => item.stock > 0).map((item) => <option key={item.variantId} value={item.variantId}>{item.productName} · {item.color} · {item.size} · saldo {item.stock}</option>)}</select></label><label><span className="text-sm font-semibold">Quantidade</span><input name="deliveredQuantity" type="number" min="1" max={delivered?.stock ?? 1} value={deliveredQuantity} onChange={(event) => setDeliveredQuantity(Number(event.target.value))} required className="ui-input mt-2" /></label><div className="rounded-xl bg-background p-4"><div className="flex justify-between text-sm text-muted"><span>Valor devolvido</span><span>{currency.format(Number(returned?.unitPrice ?? 0) * returnedQuantity)}</span></div><div className="mt-2 flex justify-between text-sm text-muted"><span>Novo produto</span><span>{currency.format(Number(delivered?.price ?? 0) * deliveredQuantity)}</span></div><div className="mt-3 flex justify-between border-t border-border pt-3 font-bold"><span>{difference > 0 ? "Diferença a receber" : difference < 0 ? "Valor a reembolsar" : "Sem diferença"}</span><span className={difference > 0 ? "text-success-strong" : difference < 0 ? "text-amber-700" : ""}>{currency.format(Math.abs(difference))}</span></div></div>{difference !== 0 ? <label><span className="text-sm font-semibold">{difference > 0 ? "Pagamento da diferença" : "Forma de reembolso"}</span><select name="paymentMethod" required className="ui-input mt-2"><option value="cash">Dinheiro</option><option value="pix">Pix</option><option value="debit_card">Débito</option><option value="credit_card">Crédito</option></select></label> : null}{difference < 0 ? <p className="text-xs leading-5 text-amber-800">O reembolso será registrado automaticamente no financeiro.</p> : null}</div></article>
    </section>
